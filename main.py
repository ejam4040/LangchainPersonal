...
def main():
    ...
    if os.environ.get('REPLIT_DB_URL'):  # Check if we're in Replit environment
        ...
        with open(output_file, "wb") as f:
            f.write(graph_image)
        print(f"Image saved as {output_file}. You can view it in the file browser.")
    else:
        try:
            # First, try using imgcat
            subprocess.run(["imgcat", temp_file], check=True)
        except (subprocess.CalledProcessError, FileNotFoundError):
            print("imgcat not available. Trying to open the image with the default viewer.")
            try:
                # On macOS
                ...
                try:
                    ...
                    print(f"Unable to display the image. It has been saved as {temp_file}")
